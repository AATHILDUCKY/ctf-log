...
import { getWriteup, incrementWriteupViews, listRelatedWriteups } from '@/lib/db/writeups';
...
export default async function WriteupPage({ params }: PageProps) {
  ...
  if (slug !== canonicalSlug) {
    redirect(`/writeups/${writeup.id}/${canonicalSlug}`);
  }
  ...
  return (
    <div className="min-h-screen bg-dracula-bg text-dracula-fg">
      ...
      />
      <Navbar />
      <main className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 py-12 sm:px-6 lg:px-8 2xl:grid-cols-[220px_minmax(0,896px)_220px]">
